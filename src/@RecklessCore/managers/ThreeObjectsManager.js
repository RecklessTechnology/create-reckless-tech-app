import { useEffect, createContext, useMemo, useState, useCallback } from 'react';
import useAppContext from '../contexts/useAppContext';

export const ThreeObjectsContext = createContext(null);
export let threeObjectsContextValue = {};

export default function ThreeObjectsManager({
  children
}) {
  ...
  }, [ThreeObjectRegistry, ThreeObjectRegistryByType, ThreeObjectRegistryByName, ThreeObjectRegistryByPos])
  useEffect(() => {
    return subscribe('scene-exit', exitScene);
  }, [subscribe, exitScene]);
  ...
  [ThreeObjectRegistry, ThreeObjectRegistryByType, ThreeObjectRegistryByName, ThreeObjectRegistryByPos, publish]
  );
  ...
  return (<ThreeObjectsContext.Provider value={threeObjectsContextValue}>{children}</ThreeObjectsContext.Provider>)
}
